...
export function RagConsentCard({ consent, language, onApprove, onDismiss }: RagConsentCardProps) {
  return (
    <div className="absolute bottom-32 left-1/2 -translate-x-1/2 w-[90%] max-w-xl bg-emerald-50 dark:bg-emerald-950 p-5 rounded-2xl border border-emerald-500/30 shadow-md z-50 animate-in slide-in-from-bottom-10 fade-in duration-300 ">
      ...
      <div className="text-sm text-slate-700 dark:text-slate-300 mb-6 font-medium bg-white/60 dark:bg-black/20 p-3 rounded-lg border border-black/5 dark:border-white/5 space-y-2">
        <div className="font-bold text-slate-900 dark:text-slate-100">{consent.title}</div>
        <p className="max-h-32 overflow-y-auto whitespace-pre-wrap">{consent.content}</p>
        <div className="inline-flex rounded-md bg-emerald-500/10 px-2 py-1 text-xs font-bold text-emerald-700 dark:text-emerald-300">
      ...
      <div className="flex gap-3">
        <button
          onClick={onApprove}
          className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-md shadow-emerald-500/20  active:scale-95 transition-all"
        >
